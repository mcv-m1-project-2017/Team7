...
directory='/home/mcv00/DataSet/train';
files = ListFiles(directory);
Method=2; %Int value: 1 if want to aply the task 1 method
...
if Method==1
    ...
    SCCL=0; % Int value: 1 if want to apply the SlidingConnectedComponentLabeling method, 0 otherwise
    SCCL_int=0;  % Int value: 1 if want to apply the SlidingConnectedComponentLabeling with integral Images method, 0 otherwise
...
elseif Method==2;
    ...
    SCCL=1; % Int value: 1 if want to apply the SlidingConnectedComponentLabeling method, 0 otherwise
    SCCL_int=0;  % Int value: 1 if want to apply the SlidingConnectedComponentLabeling with integral Images method, 0 otherwise
    TM = 0; % Int value: 1 if you want to apply the corr_template_matching method, 0 otherwise
elseif Method==3;
    CCL=0; % Int value: 1 if want to apply the ConnectedComponentLabeling method, 0 otherwise
    SCCL=0; % Int value: 1 if want to apply the SlidingConnectedComponentLabeling method, 0 otherwise
    SCCL_int=1;  % Int value: 1 if want to apply the SlidingConnectedComponentLabeling with integral Images method, 0 otherwise 
    TM = 0; % Int value: 1 if you want to apply the corr_template_matching method, 0 otherwise
elseif Method==4
    CCL=0; % Int value: 1 if want to apply the ConnectedComponentLabeling method, 0 otherwise
    SCCL=0; % Int value: 1 if want to apply the SlidingConnectedComponentLabeling method, 0 otherwise
    SCCL_int=0;  % Int value: 1 if want to apply the SlidingConnectedComponentLabeling with integral Images method, 0 otherwise 
    TM = 1; % Int value: 1 if you want to apply the corr_template_matching method, 0 otherwise
end
...
if show
  ...
  elseif SCCL
      ...
      BoundingBoxes=SlidingConnectedComponentLabeling(im_seg, 0);    %BoundingBoxes = [tly, tlx, w, h)
  elseif SCCL_int
      BoundingBoxes=SlidingConnectedComponentLabeling_int(im_seg, 0);    %BoundingBoxes = [tly, tlx, w, h)
  elseif TM
  ...
  if show
   ...
   if write_results
    imwrite(im_seg,strcat('/home/mcv07/Team7/train_windows_masks/method', num2str(Method),'/',files(i).name(1:9),'.png'));
    direc_save=strcat('/home/mcv07/Team7/train_windows_masks/method', num2str(Method),'/',files(i).name(1:9),'.mat');
