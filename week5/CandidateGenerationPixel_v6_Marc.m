...
clc 
addpath('evaluation');
directory='validation_images'; %directori of the images that we whant to mask
directory_train='train'; %directori of the mask's (for the evaluation
directory_results='masks'; %directori where the results are saved

method=3; %Int value: 1 if want to aply method 1
...
%TODO: change "CCL_Team3" to another name.
...
if method==1
   %HS + CTM + CCL(our)
    ...
    stdr=1.2; %standard deviation increase
...
if method==3
    ...
    stdr=1.5; %standard deviation increase
    cw_thresh = 0.25; % Double value: Threshold to apply to the correlation template matching in order to compare candidate windows. Between 0 and 1.
    crosscorr_thresh = 0.7; % Double value: Threshold to apply to the cross-correlation template matching method
    chanfer_th = 13750; % Int value: Threshold to apply to the chanfer_template_matching method
end
...
%Morphological operators:
    ...
    if CTM
         [im_seg] = chamfer_template_matching(im_seg, chanfer_templates, chanfer_th);
...
%Connected Component Labeling:    
    ...
    if CCL_Team3
        ...
        [im_seg,windowCandidates] = CCL_filtering(im_seg, features.features);
    end
