function [bboxes] = SlidingConnectedComponentLabeling_int(im_seg, show)
...
function [X, Y] = find_candidates(img, size_i, size_j, fr, th)
    ...
    X = []; Y= [];
    step = 10;
    %Reorder values from the image
    ii = cumsum(cumsum(double(img)), 2);
    %sliding process
    for i = 1:step:h-size_i
        for j = 1:10:w-size_j
            filling_ratio = integral_image(ii,i,j,size_i,size_j)/(size_i*size_j);
